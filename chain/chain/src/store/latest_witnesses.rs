...
impl ChainStore {
    ...
    pub fn save_latest_chunk_state_witness(
        ...
        tracing::debug!(
            "Generating store update to save latest witness took {:?}",
            store_update_time
        );
        ...
        tracing::debug!(
            "Committing store update to save latest witness took {:?}",
            start_time.elapsed().saturating_sub(store_update_time)
        );
