...
impl Client {
    ...
    pub fn finish_blocks_in_processing(&mut self) -> Vec<CryptoHash> {
        ...
        while wait_for_all_blocks_in_processing(&mut self.chain) {
            accepted_blocks.extend(
                self.postprocess_ready_blocks(None, true, &signer)
                    .0,
            );
        }
        accepted_blocks
    ...
    /// has started.
    pub fn finish_block_in_processing(
        &mut self,
        hash: &CryptoHash,
    ) -> Vec<CryptoHash> {
...
pub fn create_chunk(
    ...
    let last_block = client
        .chain
        .get_block_by_height(client.chain.head().unwrap().height)
        .unwrap();
    ...
    if should_replace {
        ...
        let total_parts = client
            .chain
            .epoch_manager
            .num_total_parts();
        let data_parts = client
            .chain
            .epoch_manager
            .num_data_parts();
    ...
    loop {
            ...
            .drain(..)
        {
            let results = do_apply_chunks(msg.block_hash, msg.block_height, msg.work)
                ...
                .map(|res| res.1)
                .collect_vec();
            if let Some(CatchupState { catchup, .. }) = client
                .catchup_state_syncs
