...
impl EncodedChunksCache {
    ...
    fn remove_chunk_from_incomplete_chunks(
        ...
        if let Occupied(mut entry) = self
            .incomplete_chunks
    ...
    pub fn height_within_front_horizon(
        &self,
        height: BlockHeight,
    ) -> bool {
    ...
    pub fn height_within_rear_horizon(
        &self,
        height: BlockHeight,
    ) -> bool {
    ...
    pub fn remove_from_cache_if_outside_horizon(
        &mut self,
        chunk_hash: &ChunkHash,
    ) {
    ...
    pub fn mark_chunk_for_inclusion(
        &mut self,
        chunk_hash: &ChunkHash,
    ) -> bool {
        let entry = self
...
mod tests {
    ...
    fn test_incomplete_chunks() {
        ...
        assert_eq!(
            cache
                .get_incomplete_chunks(&CryptoHash::default())
                .unwrap(),
            &HashSet::from([
                header0.chunk_hash(),
                header1.chunk_hash()
    ...
    fn test_cache_removal() {
        ...
        let partial_encoded_chunk =
            PartialEncodedChunkV2 { header, parts: vec![], prev_outgoing_receipts: vec![] };
