...
impl DataSizeDistribution {
    fn new(
        ...
        let total_num_of_pairs = key_sizes
            .iter()
            .map(|(_, count)| count)
            .sum::<usize>();

        Self { key_sizes, value_sizes, total_num_of_pairs, column_families_data: col_families_data }
    }
    ...
    fn print_sizes_count(
        ...
        let total_sizes_bytes_sum = sizes_count
            .iter()
            .map(|a| a.0 * a.1)
            .sum::<usize>();
        println!(
...
fn read_all_pairs(
    ...
    col_families
        ...
        .for_each(|col_family| {
...

            {
                ...
                let column_size = local_key_sizes
                    .iter()
    ...
    let key_sizes: Vec<(usize, usize)> = key_sizes
        .lock()
        .unwrap()
        .clone()
        .into_iter()
        .collect();
    let value_sizes: Vec<(usize, usize)> = value_sizes
        .lock()
        .unwrap()
        .clone()
        .into_iter()
        .collect();
    let column_families: Vec<(String, ColumnFamilyCountAndSize)> = column_families_data
        .lock()
        .unwrap()
        .clone()
        .into_iter()
