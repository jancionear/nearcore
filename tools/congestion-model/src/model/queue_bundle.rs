...
impl QueueBundle {
    pub fn new(shards: &[ShardId]) -> Self {
        ...
        for &shard in shards {
            ...
            this.shard_mailbox
                .insert(shard, mailbox);
            this.transaction_queues
                .insert(shard, VecDeque::new());
        }
    ...
    pub fn incoming_transactions(
        &self,
        shard_id: ShardId,
    ) -> &VecDeque<TransactionId> {
        self.transaction_queues
            .get(&shard_id)
            .expect("transaction queue should exist for all shards")
    }

    pub fn incoming_transactions_mut(
        &mut self,
        shard_id: ShardId,
    ) -> &mut VecDeque<TransactionId> {
        self.transaction_queues
            .get_mut(&shard_id)
            .expect("transaction queue should exist for all shards")
    }
    ...
    pub fn shard_queues(
        &self,
        shard_id: ShardId,
    ) -> impl Iterator<Item = &Queue> {
        self.receipt_queues
            .iter()
            .filter(move |q| q.shard() == shard_id)
    }
}
