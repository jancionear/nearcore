...
impl Model {
    ...
    pub fn write_stats_header(
        ...
        stats_writer
            .write_field("finished_transactions")
            .unwrap();
        stats_writer
            .write_field("pending_transactions")
            .unwrap();
        stats_writer
            .write_field("waiting_transactions")
            .unwrap();
        stats_writer
            .write_field("failed_transactions")
            .unwrap();
        ...
        for shard_id in self.shard_ids.clone() {
            for queue in self.queues.shard_queues(shard_id) {
                ...
                stats_writer
                    .write_field(format!("{field_name}_len"))
                    .unwrap();
                stats_writer
                    .write_field(format!("{field_name}_bytes"))
                    .unwrap();
                stats_writer
                    .write_field(format!("{field_name}_gas"))
                    .unwrap();
            }
        }

        stats_writer
            .write_field("successful_tx_delay_avg")
            .unwrap();
        stats_writer
            .write_field("successful_tx_delay_median")
            .unwrap();
        stats_writer
            .write_field("successful_tx_delay_90th_percentile")
            .unwrap();
        stats_writer
            .write_field("rejected_tx_delay_avg")
            .unwrap();
        stats_writer
            .write_field("rejected_tx_delay_median")
            .unwrap();
        stats_writer
            .write_field("rejected_tx_delay_90th_percentile")
            .unwrap();
        stats_writer
            .write_field("unresolved_transactions")
            .unwrap();
    ...
    pub fn write_stats_values(
        ...
        stats_writer
            .write_field(format!("{:?}", time))
            .unwrap();
        stats_writer
            .write_field(format!("{}", round))
            .unwrap();

        // This is slow and takes up to 10s for the slowest workloads and strategies.
        let progress = self.progress();
        stats_writer
            .write_field(format!("{}", progress.finished_transactions))
            .unwrap();
        stats_writer
            .write_field(format!("{}", progress.pending_transactions))
            .unwrap();
        stats_writer
            .write_field(format!("{}", progress.waiting_transactions))
            .unwrap();
        stats_writer
            .write_field(format!("{}", progress.failed_transactions))
            .unwrap();

        for shard_id in self.shard_ids.clone() {
            for queue in self.queues.shard_queues(shard_id) {
                stats_writer
                    .write_field(format!("{}", queue.len()))
                    .unwrap();
                stats_writer
                    .write_field(format!("{}", queue.size()))
                    .unwrap();
                stats_writer
                    .write_field(format!("{}", queue.attached_gas()))
                    .unwrap();
            }
        }
        ...
        stats_writer
            .write_field(
                user_exp
                    .successful_tx_delay_90th_percentile
                    .to_string(),
            )
            .unwrap();
        stats_writer
        ...
        stats_writer
            .write_field(
                user_exp
                    .rejected_tx_delay_90th_percentile
                    .to_string(),
            )
            .unwrap();
        stats_writer
