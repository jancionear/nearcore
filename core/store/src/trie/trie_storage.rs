...
impl TrieCacheInner {
    ...
    pub(crate) fn put(
        ...
        while self.total_size > self.total_size_limit || self.cache.len() == self.cache.cap().get()
        {
            ...
            let (_, value) = self
                .cache
                .pop_lru()
                .expect("Cannot fail because total size capacity is > 0");
            self.remove_value_of_size(value.len());
        }
    ...
    pub(crate) fn add_value_of_size(
        &mut self,
        len: usize,
    ) {
    ...
    pub(crate) fn remove_value_of_size(
        &mut self,
        len: usize,
    ) {
        self.total_size -= Self::entry_size(len);
...
mod trie_cache_tests {
    ...
    fn test_size_limit() {
        ...
        let mut cache =
            TrieCacheInner::new(100, value_size_sum + memory_overhead, ShardId::new(0), false);
    ...
    fn test_deletions_queue() {
        let mut cache = TrieCacheInner::new(2, 1000, ShardId::new(0), false);
    ...
    fn test_cache_capacity() {
        ...
        let mut cache = TrieCacheInner::new(100, total_size_limit, ShardId::new(0), false);
        put_value(&mut cache, &[1]);
    ...
    fn test_trie_config() {
        ...
        store_config
            .trie_cache
            .default_max_bytes = DEFAULT_SIZE;
        store_config
            .trie_cache
            .per_shard_max_bytes
            .insert(s0, S0_SIZE);
        store_config
            .view_trie_cache
            .default_max_bytes = DEFAULT_VIEW_SIZE;
        store_config
            .view_trie_cache
            .per_shard_max_bytes
            .insert(s0, S0_VIEW_SIZE);
