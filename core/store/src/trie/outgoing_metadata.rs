...
impl ReceiptGroupsQueue {
    ...
    fn load_data(
        ...
        let data_opt: Option<ReceiptGroupsQueueData> =
            get(trie, &TrieKey::OutgoingReceiptGroupsQueueData { receiver_shard })?;
    ...
    fn save_data(&self, state_update: &mut TrieUpdate) {
        ...
        set(
            state_update,
            TrieKey::OutgoingReceiptGroupsQueueData { receiver_shard: self.receiver_shard },
            &data_enum,
        );
    }
...
impl TrieQueue for ReceiptGroupsQueue {
    ...
    fn trie_key(&self, index: u64) -> TrieKey {
        TrieKey::OutgoingReceiptGroupsQueueItem { receiver_shard: self.receiver_shard, index }
    }
}
