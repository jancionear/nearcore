...
fn test_chain_with_dynamic_resharding_at_genesis() {
    ...
    let (genesis, epoch_config) = genesis_with_dynamic_resharding(&shard_layout, validators_spec);
...
/// startup in `nearcore::start_with_config_and_synchronization_impl`.
#[test]
fn test_genesis_state_init_with_dynamic_resharding_at_genesis() {
    ...
    let (genesis, epoch_config) =
        genesis_with_dynamic_resharding(&shard_layout, create_validators_spec(2, 0));
