    ...
    pub(crate) fn new(
        ...
        if let Some(own_congestion_info) = prev_own_congestion_info {
            ...
            let outgoing_buffer_metadatas = OutgoingMetadatas::load(
                ...
                50_000, // TODO(bandwidth_scheduler) - put in config
...
impl ReceiptSinkV2 {
    ...
    fn forward_from_buffer_to_shard(
            ...
            self.outgoing_buffers.to_shard(shard_id).iter(&state_update.trie, true)
        {
            ...
            let should_update_outgoing_metadaa = receipt.should_update_outgoing_metadata();
            ...
            match Self::try_forward(
                ...
                ReceiptForwarding::Forwarded => {
                    ...
                    if should_update_outgoing_metadaa {
    ...
    fn generate_bandwidth_request(
        ...
        let have_full_metadata = first_receipt.should_update_outgoing_metadata();

        let request = if have_full_metadata {
            let metadata = self
                .outgoing_buffer_metadatas
                .metadatas
                .get(&to_shard)
                .expect("Outgoing buffer metadata should exist for shard with full metadata");
            let mut receipt_group_sizes = metadata.receipt_group_sizes();
