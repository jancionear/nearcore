...
pub fn client_tracking_shard(
    ...
    client
        .shard_tracker
        .care_about_shard(account_id, parent_hash, shard_id, true)
}
...
pub fn print_and_assert_shard_accounts(
    clients: &[&Client],
    tip: &Tip,
) {
    let epoch_config = clients[0]
        .epoch_manager
...
pub fn this_block_has_new_shard_layout(
    epoch_manager: &dyn EpochManagerAdapter,
    tip: &Tip,
) -> bool {
    if !epoch_manager
        .is_next_block_epoch_start(&tip.prev_block_hash)
        .unwrap()
    {
        return false;
    }

    let prev_epoch_id = epoch_manager
        .get_epoch_id(&tip.prev_block_hash)
        .unwrap();
    let this_epoch_id = epoch_manager
        .get_epoch_id(&tip.last_block_hash)
        .unwrap();

    let prev_shard_layout = epoch_manager
        .get_shard_layout(&prev_epoch_id)
        .unwrap();
    let this_shard_layout = epoch_manager
        .get_shard_layout(&this_epoch_id)
        .unwrap();
...
// shard layouts.
pub fn next_block_has_new_shard_layout(
    epoch_manager: &dyn EpochManagerAdapter,
    tip: &Tip,
) -> bool {
    if !epoch_manager
        .is_next_block_epoch_start(&tip.last_block_hash)
        .unwrap()
    {
        return false;
    }
...
pub fn next_epoch_has_new_shard_layout(
    epoch_manager: &dyn EpochManagerAdapter,
    tip: &Tip,
) -> bool {
    let this_shard_layout = epoch_manager
        ...
        .unwrap();
    let next_shard_layout = epoch_manager
        .get_shard_layout(&tip.next_epoch_id)
        .unwrap();

    this_shard_layout != next_shard_layout
}

pub fn shard_was_split(
...
pub fn get_shards_will_care_about(
    client: &Client,
    block_hash: &CryptoHash,
) -> Vec<ShardUId> {
    let signer = client.validator_signer.get();
