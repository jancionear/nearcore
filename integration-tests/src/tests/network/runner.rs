...
fn setup_network_node(
    ...
    client_config.ttl_account_id_router = config
...
async fn check_routing_table(
    ...
    let want_rt: Vec<_> = want
        ...
        .map(|(target, routes)| {
            let peers = routes
                .into_iter()
                .map(|hop| info.runner.test_config[hop].peer_id())
                .collect();
...
impl Runner {
    pub fn new(
        ...
        let test_config: Vec<_> = (0..num_nodes)
            .map(TestConfig::new)
            .collect();
        let validators = test_config[0..num_validators]
            .iter()
    ...
    pub fn minimum_outbound_peers(
        mut self,
        minimum_outbound_peers: u32,
    ) -> Self {
        self.apply_all(move |test_config| {
    ...
    async fn setup_node(
        ...
        let boot_nodes = config
            .boot_nodes
            .iter()
            .map(|ix| self.test_config[*ix].peer_info())
            .collect();
...
pub(crate) fn start_test(runner: Runner) -> anyhow::Result<()> {
    ...
    r.block_on(async {
        ...
        for (i, a) in actions.into_iter().enumerate() {
            ...
            loop {
                let done = tokio::time::timeout_at(start + timeout, a(&mut info))
                    .await
                    .with_context(|| {
...
impl RunningInfo {
    fn get_node(
        ...
        self.nodes[node_id]
            .as_ref()
            .ok_or_else(|| anyhow!("node is down"))
    }
    fn stop_node(
        &mut self,
        node_id: usize,
...
pub(crate) fn assert_expected_peers(
    node_id: usize,
    peers: Vec<usize>,
) -> ActionFn {
    Box::new(move |info: &mut RunningInfo| {
        ...
        Box::pin(async move {
            ...
            let got: HashSet<_> = network_info
                ...
                .collect();
            let want: HashSet<_> = peers
                .iter()
                .map(|i| info.runner.test_config[*i].peer_id())
                .collect();
            if got != want {
...
pub(crate) fn change_account_id(
    node_id: usize,
    account_id: AccountId,
) -> ActionFn {
    Box::new(move |info: &mut RunningInfo| {
