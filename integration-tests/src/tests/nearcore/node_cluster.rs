...
fn start_nodes(
    ...
    let mut genesis = Genesis::test_sharded_new_version(
        ...
        num_validator_seats,
        (0..num_shards)
            .map(|_| num_validator_seats)
            .collect(),
    );
    genesis.config.epoch_length = epoch_length;
    genesis.config.genesis_height = genesis_height;

    let validators = (0..num_validator_seats)
        .map(|i| format!("near.{}", i))
        .collect::<Vec<_>>();
    ...
    for i in 0..num_nodes {
        ...
        if i >= num_validator_seats && i < num_tracking_nodes {
            near_config.client_config.tracked_shards = vec![ShardId::new(0)];
        }
...
impl NodeCluster {
    ...
    pub fn set_num_validator_seats(
        mut self,
        n: NumSeats,
    ) -> Self {
        ...
        ) -> R,
    {
        let (num_shards, num_validator_seats, num_lightclient, epoch_length, genesis_height) = (
            self.num_shards
                .expect("cluster config: [num_shards] undefined"),
            self.num_validator_seats
                .expect("cluster config: [num_validator_seats] undefined"),
            self.num_lightclient
                .expect("cluster config: [num_lightclient] undefined"),
            self.epoch_length
                .expect("cluster config: [epoch_length] undefined"),
            self.genesis_height
                .expect("cluster config: [genesis_height] undefined"),
        );
