...
fn ultra_slow_test_sync_state_nodes() {
    heavy_test(|| {
        ...
        near1
            .network_config
            .peer_store
            .boot_nodes = convert_boot_nodes(vec![]);
    ...
    heavy_test(|| {
        ...
        run_actix(async move {
            ...
            near1
                .network_config
                .peer_store
                .boot_nodes = convert_boot_nodes(vec![("test3", *port3), ("test4", *port4)]);
            ...
            near1
                .client_config
                .max_block_production_delay = Duration::milliseconds(400);

            let mut near3 = load_test_config("test3", port3, genesis.clone());
            near3
                .network_config
                .peer_store
                .boot_nodes = convert_boot_nodes(vec![("test1", *port1), ("test4", *port4)]);
            near3.client_config.min_num_peers = 2;
            near3
                .client_config
                .min_block_production_delay = near1
                .client_config
                .min_block_production_delay;
            near3
                .client_config
                .max_block_production_delay = near1
                .client_config
            ...
            near4
                .network_config
                .peer_store
                .boot_nodes = convert_boot_nodes(vec![("test1", *port1), ("test3", *port3)]);
...
fn ultra_slow_test_sync_state_dump() {
    heavy_test(|| {
        ...
        run_actix(async move {
            ...
            near1
                .client_config
                .min_block_production_delay = Duration::milliseconds(300);
            near1
                .client_config
                .max_block_production_delay = Duration::milliseconds(600);
                        ...
                        .await
                    {
                        ...
                        | Ok(Ok(b)) if b.header.height >= genesis.config.epoch_length + 2 => {
                            ...
                            if view_client2_holder2.is_none() {
                                ...
                                near2
                                    .client_config
                                    .state_sync_external_timeout = Duration::seconds(2);
                                near2
                                    .client_config
                                    .state_sync_p2p_timeout = Duration::seconds(2);
...
fn slow_test_state_sync_headers() {
    heavy_test(|| {
        ...
        let _dir1 = Arc::new(
            tempfile::Builder::new()
                .prefix("test_state_sync_headers")
                .tempdir()
                .unwrap(),
        );
        let dir1 = _dir1.clone();
        ...
        run_actix(async {
            ...
            wait_or_timeout(1000, 110000, || async {
                let epoch_id = match view_client1
                    .send(GetBlock::latest().with_span_context())
                    .await
                {
...
fn slow_test_state_sync_headers_no_tracked_shards() {
    heavy_test(|| {
        ...
        run_actix(async {
            ...
            // Third, we request state sync part with part_id = 0.
            wait_or_timeout(1000, 110000, || async {
                let epoch_id = match view_client2
                    .send(GetBlock::latest().with_span_context())
                    .await
                {
                    | Ok(Ok(b)) => Some(b.header.epoch_id),
