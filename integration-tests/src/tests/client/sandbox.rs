...
fn test_patch_state() {
    ...
    let state_item = env
        .query_state("test0".parse().unwrap())
        .swap_remove(0);
    env.clients[0]
        .chain
        .patch_state(SandboxStatePatch::new(vec![StateRecord::Data {
...
fn test_patch_account() {
    let (mut env, _signer) = test_setup();
    let mut test1: Account = env
        .query_account("test1".parse().unwrap())
        .into();
    test1.set_amount(10);

    env.clients[0]
        .chain
        .patch_state(SandboxStatePatch::new(vec![StateRecord::Account {
