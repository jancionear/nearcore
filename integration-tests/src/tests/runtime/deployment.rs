...
fn test_deploy_max_size_contract() {
    ...
    let test_contract_id: AccountId = "test_contract.alice.near"
        .parse()
        .unwrap();
    ...
    let signed_transaction = SignedTransaction::from_actions(
        ...
        vec![Action::DeployContract(
            DeployContractAction { code: vec![0u8] },
        )],
        block_hash,
        0,
    );
    ...
    .unwrap();
    let transaction_result = node_user
        .deploy_contract(test_contract_id, wasm_binary.to_vec())
        .unwrap();
    assert_eq!(transaction_result.status, FinalExecutionStatus::SuccessValue(Vec::new()));
    assert_eq!(
        transaction_result
    ...
    let tx_conversion_gas_burnt = transaction_result
        .transaction_outcome
        .outcome
        .gas_burnt;
    let deployment_gas_burnt = transaction_result.receipts_outcome[0]
        .outcome
        .gas_burnt;
